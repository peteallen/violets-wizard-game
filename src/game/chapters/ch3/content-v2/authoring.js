...
export function learningStart(id) {
  return { type: 'learning.start', id };
}

export function spellLearn(spell) {
  return { type: 'spell.learn', spell };
}
...
export function sfxCue(at, key, { pan } = {}) {
  return {
    type: 'cue',
    at,
    event: 'audio.command',
    payload: { command: 'sfx', key, ...(pan === undefined ? {} : { pan }) },
  };
}
