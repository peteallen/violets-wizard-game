...
describe('character image generation', () => {
  ...
  it('makes one generation request and atomically saves validated PNG plus safe provenance', async () => {
    ...
    expect(provenance.response.body_without_image_bytes.data[0].image_bytes).toEqual({
      bytes: generatedPng.length,
      sha256: hash(generatedPng),
    });
    expect(provenance.response.body_without_image_bytes.provider_metadata.echoed_secret).toBe('hidden [REDACTED]');
