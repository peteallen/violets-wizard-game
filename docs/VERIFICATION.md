...
- **Asset gating before a reviewed frame**: explicit `FontFace.load()` per face/weight actually used (`document.fonts.ready` alone is insufficient — it resolves early if nothing triggered a load) + one warm-up `fillText` offscreen; `await img.decode()` for every image visible in that composition; **audio hard-stubbed in harness mode** (headless AudioContext never resumes; anything awaiting it hangs the harness). Production uses the same composition-scoped boundary for the title, adventure entry, room transitions, and set pieces instead of globally preloading later chapters.
...
Startup and recovery have explicit review coverage even though the earliest state exists before the ordinary Canvas harness can boot. `boot-review.html` renders the real pre-module surface for `boot-loading-review` and `boot-failure-review`, and `snap-dom.mjs` captures it as a complete viewport. The in-game `composition-loading-review` and `composition-failure-review` fixtures exercise the production Canvas overlay while holding a complete underlying frame. All four are captured at 1280×720 and 2560×1440. Their checklists require loading meaning without motion, a failure treatment that cannot be confused with gold selection, and one obvious Retry action where recovery is possible.
...
- Canvas captures standardize on `__snapshot()`/toDataURL, **not** `page.screenshot` — verified pixel-identical at dpr=1 today, but toDataURL reads the raw backing store (immune to future CSS scaling) and keeps all frames RGBA, which matters because of a **verified ffmpeg gotcha**: a PNG sequence mixing RGBA and RGB frames makes the image2 demuxer silently blank frames in GIF/tile output. Registered accessible DOM overlays are the explicit exception: `snap-dom.mjs` takes a deterministic viewport screenshot after the same seeded harness step so the modal, native input, focus state, and Canvas beneath it are reviewed together.
...
### Slow-network browser proof

Deterministic fixtures prove the loading and failure compositions, but they cannot prove request ordering or recovery across the network boundary. A delivery change therefore also gets a cold production-preview run in Chromium at 400 kbps with 400 ms latency. Inspect the page from navigation onward: the pre-module boot surface must appear before the runtime arrives; the title may not replace it until its backdrop, action, and one visible Violet pose form a complete frame; idle and blink poses may load later; and entering a room or set piece must retain the previous complete frame rather than exposing procedural art. Network inspection confirms that later chapter presentation/content and non-visible character poses are absent from the title-critical request path.

The same run deliberately aborts one required image request. The corresponding failure state must become visible, its Retry action must issue a fresh request, and recovery must create only one live game. Music, voice, incoming simulation time, and set-piece time remain stopped until the retried composition is ready. During a held title-to-room request, verify the Canvas retains its full backing and client dimensions, `window.__harness` remains absent, and pixels outside the notice equal the complete previous frame under the documented veil. After the emitted service worker has installed, stop the preview server or otherwise remove the worker's own network access, disable the ordinary HTTP cache, and reload once more; the shell and title must still become ready from the current revision cache. Confirm that `version.json`, review chunks, `harness.html`, range requests, and unvisited chapter assets are not swallowed into that cache, and that an unavailable `harness.html` navigation fails rather than receiving `index.html`. Browser proof complements the registered captures and tests; it does not replace either one.

## The tools

| Script | Does |
|---|---|
| `scripts/snap.mjs` | `--scene <id> --times 0,0.25,0.5,1,2 --seed 42` → starts Vite **programmatically** (`createServer()` — no port races or orphan processes when agents run captures concurrently), loads the harness once, iterates `__renderAt(t)` + `__snapshot()`, writes `review/<scene>/.../seed42_f000015.png` (zero-padded frames so globs sort chronologically) |
| `scripts/snap-dom.mjs` | Uses the same selectors, deterministic frames, environment identity, names, and `capture.json` schema as `snap.mjs`, but captures the complete viewport for registered DOM surfaces such as pet naming and save transfer. It is opt-in so Canvas goldens never silently change capture paths. |
