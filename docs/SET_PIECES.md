...
Painted backgrounds are *never* animated, warped, or partially redrawn. Anything that moves — characters, props, doors, bricks, letters, flames, reflections — lives in the code-drawn layer or in a **pre-sliced piece of painting** that code moves as a rigid tile. This is why the art rule "no people, no creatures, no text in backgrounds" is load-bearing: it's not a style preference, it's the animation architecture. The painting is the stage; everything on the stage is ours.
...
3. *The animated grid:* at build time, a slice map (JSON, hand-tuned once) defines a ~10×8 grid over the wall region with slight per-tile size jitter so cuts land on mortar lines where possible. **At room load, tiles are pre-sliced into individual small sprite canvases** (integer source rects, each padded with 1–2px of duplicated edge pixels) — never re-sampled from the 2048-wide source per frame; red-team confirmed fresh source-rect sampling under scale+rotation produces edge fringing, and ~80 pre-sliced `drawImage` calls per frame cost <1ms on iPad-class GPUs. Tiles draw with ~1% overlap scale, and tiles that haven't started moving stay snapped to integer device pixels — both measures kill the hairline antialiased cracks that would otherwise show the alley through a "solid" wall. Animation: tiles rotate away around their outer edge and slide outward+back with `easeInOutCubic`, ordered by distance from the tap point (spiral wave, 60ms stagger), each fading the last 20% of its travel. **The intact wall painting stays drawn *beneath* the tile grid**, and the reveal painting is uncovered per-tile only as each tile actually departs — so any residual seam shows wall pixels, never a premature glimpse of the alley.
...
**Assets:** courtyard painting (center wall), slice-map JSON, rumble/clack SFX. ~2.5s total, ~80 tiles = ~80 transformed `drawImage` calls/frame at peak (well inside budget; brick-slicing looks *intentional* because bricks are rectangles).
