...
3. *The animated grid:* an authored 10×8 mortar grid defines 80 wall cells. The courtyard remains one decoded painting used only as the source for the intact wall cells: untouched cells use overlapping nine-argument `drawImage` crops so the wall has no premature seams, while each departing cell switches to one isolated painted brick face with soft corners rather than carrying a rectangular slice of several bricks. **Do not allocate a canvas per brick.** That would violate the global five-canvas WebKit budget even though each canvas is small. From set-piece frame zero, Diagon Alley is drawn once at its final full-canvas world scale and never zooms, but it is clipped to the wall bounds so the surrounding courtyard stays unchanged. The wall cells sit over that hidden destination until their movement exposes it through the gaps. A post-cast pass then expands the same stable painting across the room to naturally occlude Violet and Hagrid, and redraws the still-moving bricks above them. Animation: the opaque bricks rotate away and slide outward+back with `easeInOutCubic`, ordered by distance from the tap point (center-out wave, 60ms distance stagger), then disappear without ghost transparency.
**Dust particles** sample every third tile's departure edge; low rumble + brick-clack ticks synced to the stagger; camera: hold 0.5s → wall opens onto the already-stable street → one irregular aperture expands from the wall until it covers the viewport and outgoing cast.
**Assets:** courtyard painting (center wall), authored grid bounds and isolated brick-face sample crops from that same painting, rumble/clack SFX. Three seconds total, at most 80 transformed source-crop `drawImage` calls per peak frame and zero additional tile canvases; dust is sampled to keep the combined draw count bounded.
**Beats:** t0 Hagrid tap-tap-tap (reviewed action clip) with the courtyard unchanged outside the wall → t0.4 wall shiver (grid jitters ±2px) → t0.8 wave starts at the center brick and exposes the full-scale street through the gaps → t1.45–2.15 the same street aperture expands across the outgoing room and cast behind the still-moving bricks → t2.2 last brick gone → t2.6 street room becomes active directly, with no second transition wipe.
...
Room-to-room and chapter cards transition via **ink blooms**: nine overlapping authored dry-brush bands sweep from the tapped doorway across the captured outgoing room, hand off through one frame of visibly layered ink wash, then sweep away from the fully prepared destination. The two rooms are never composited side by side. Each band has a different curved, broken end plus short pigment fibers, and the full sequence lasts 0.8s. Map travel keeps the faster sparkle-cover variant. **Verify:** the outgoing room is fully inked before any destination pixel appears; the cover reads as textured ink rather than a blank screen; the reveal edge is asymmetric and materially brushed at 2× zoom; no circular iris, repeating scallops, exposed painting boundary, or white flash.
