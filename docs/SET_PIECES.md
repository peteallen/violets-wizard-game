...
3. *The animated grid:* an authored 10×8 mortar grid defines 80 wall cells. The courtyard remains one decoded painting used only as the source for the intact wall cells: untouched cells use overlapping nine-argument `drawImage` crops so the wall has no premature seams, while each departing cell switches to one isolated painted brick face with soft corners rather than carrying a rectangular slice of several bricks. **Do not allocate a canvas per brick.** That would violate the global five-canvas WebKit budget even though each canvas is small. From set-piece frame zero, Diagon Alley is drawn once at its final full-canvas world scale and never zooms; the wall cells sit over it, so there is no courtyard frame around the destination. A post-cast pass redraws that same stable painting only to let the aperture naturally occlude Violet and Hagrid, then redraws the still-moving bricks above them. Animation: the opaque bricks rotate away and slide outward+back with `easeInOutCubic`, ordered by distance from the tap point (center-out wave, 60ms distance stagger), then disappear without ghost transparency.
**Dust particles** sample every third tile's departure edge; low rumble + brick-clack ticks synced to the stagger; camera: hold 0.5s → wall opens → one irregular aperture grows around the already-stable, full-scale street painting until it covers the viewport and outgoing cast.
...
**Beats:** t0 Hagrid tap-tap-tap (reviewed action clip) → t0.4 wall shiver (grid jitters ±2px) → t0.8 wave starts at the center brick with the full-scale street already behind it → t1.45–2.15 the same street aperture occludes the outgoing room and cast behind the still-moving bricks → t2.2 last brick gone → t2.6 street room becomes active directly, with no second transition wipe.
