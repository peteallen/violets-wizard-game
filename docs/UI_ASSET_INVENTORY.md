...
| Loading and recovery | Pre-module storybook boot; staged loading copy; boot failure and Retry; in-game composition loading and failure overlays | Shipped live-rendered recovery family | Keep `boot-loading-review`, `boot-failure-review`, `composition-loading-review`, and `composition-failure-review` current at both required sizes. Loading meaning must survive reduced motion, failure must remain distinct from selection and enchanted gold, and Retry must be the one obvious action. In play, these surfaces must retain the last complete title or room frame until the incoming composition is wholly ready; they must never certify a mixed painted/procedural intermediate frame. |
